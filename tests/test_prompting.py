...
def test_router_input_prefers_local_heatmap_image(tmp_path: Path) -> None:
    ...
    assert "현재 미체결 주문이 있으면" in str(content[0]["text"])
    structured_context = str(content[2]["text"])
    assert '"current_price"' in structured_context
    assert '"position_side"' in structured_context
    assert '"active_orders"' in structured_context
    assert '"has_active_entry_workflow"' in structured_context
