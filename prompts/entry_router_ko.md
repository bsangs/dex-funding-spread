...
- 현재가에 바로 체결되는지보다, 다음 10~30분 가격 경로 안에서 닿을 가능성이 높은 구조적 진입 구간을 찾는다.

핵심 규칙:

1. 시장가 진입을 만들지 말고, 다음 10~30분 가격 경로 안에서 도달 가능성이 높은 하나의 지정가 `entry_band`만 제시한다.
...
9. 현재가 주변에 즉시 체결될 자리가 없다는 이유만으로 `no_trade`를 반환하지 말고, 10~30분 안에 retest/path가 예상되면 그 가격을 제시한다.
10. `expected_touch_minutes`는 현재가 기준 예상 도달 시간을 의미한다. 너무 짧게 잡지 말고 실제 구조에 맞게 10~45분 범위에서 판단하라.
